"""
Gradient Specificity Evaluation — Rule-Dilution Diagnostic (RQ3)

Measures how focused each textual gradient is on its target task, vs being
diluted with generic or cross-task advice. This is the CARO rule-dilution
analog: combined gradients (SCC/CCC) should be less task-focused than
separate gradients (SSS/SSC/Single).

Runs on SummEval val=mae and val=none, all modes, all 3 runs.
...
SPECIFICITY_PROMPT = """\
You are measuring how focused a piece of textual feedback (called a \
"gradient") is on a specific evaluation task, versus being diluted with \
generic advice or advice that belongs to other tasks.

The target task is "{task}". The possible tasks are: fluency, relevance, \
    ...
    for step in range(1, num_steps + 1):
        ...
        for task in task_names:
            ...
            records.append({
                "step": step,
                "task": task,
                "eval_prompt": SPECIFICITY_PROMPT.format(task=task, gradient_text=gradient_text),
            })
    return records
...
def main():
    ...
    out_path = os.path.join(OUT_DIR, "gradient_specificity.parquet")
    ...
    print_analysis(combined, metric_name="Gradient Specificity")
