...
from when_gradients_collide.config import WgcDefaults, wgc_config, temp_config
...
def _make_pe2(**overrides):
    from when_gradients_collide.algorithm import PE2
...
class TestGlobalConfigTemperatureFallback:
    ...
    def test_temp_config_still_works_for_temperatures(self):
        original = wgc_config.defaults.task_llm_temperature
        with temp_config(task_llm_temperature=1.5):
            assert wgc_config.defaults.task_llm_temperature == 1.5
        assert wgc_config.defaults.task_llm_temperature == original
