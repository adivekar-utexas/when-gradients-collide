"""Run TextGrad val=none experiments: runs 2 and 3.

Job groups (val=none only, no mae):
1. Multi-task, val=none, 4 modes (SSS/SSC/SCC/CCC) — runs 2 & 3
2. Single-task, val=none, CCC mode, one task at a time — runs 2 & 3
...
def build_jobs(ds_config: Dict[str, Any]) -> List[JobSpec]:
    ...
    for run_n in RUNS:
        for mode_name in ["SSS", "SSC", "SCC", "CCC"]:
            job_name: str = (
                f"TextGrad-{ds_name}-{DATE_PREFIX}-{mode_name}-val=none-run_{run_n}"
            )
        ...
        for task_name in all_tasks:
            job_name = (
                ...
                f"-val=none-task={task_name}-run_{run_n}"
            )
            jobs.append((job_name, MODE_CONFIGS["CCC"], [task_name], [task_name]))
...
def run_dataset(ds_config: Dict[str, Any]) -> None:
    ...
    print(f"\n{'=' * 90}")
    print(f"Dataset: {ds_name} — Launching {len(jobs)} parallel jobs (runs {RUNS})")
    print(f"{'=' * 90}")
    ...
    max_workers: int = min(len(jobs), 20)
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        futures = {
            executor.submit(
                run_single, job_name, mode_cfg, ds_class, subset,
            ): (job_name, eval_tasks)
