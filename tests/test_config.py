"""
Unit tests for when_gradients_collide.config — centralized configuration system.
...
from when_gradients_collide.config import (
    WgcConfig,
    WgcDefaults,
    wgc_config,
    temp_config,
...
class TestWgcConfigMutation:
    ...
    def test_mutate_and_read(self):
        ...
        original = wgc_config.defaults.task_llm_timeout
        try:
            wgc_config.defaults.task_llm_timeout = 999.0
            assert wgc_config.defaults.task_llm_timeout == 999.0
        finally:
            wgc_config.defaults.task_llm_timeout = original

    def test_validation_rejects_negative_timeout(self):
        """Pydantic validation prevents invalid values."""
        with pytest.raises(Exception):
            wgc_config.defaults.task_llm_timeout = -1.0

    def test_validation_rejects_temperature_above_2(self):
        with pytest.raises(Exception):
            wgc_config.defaults.task_llm_temperature = 3.0

    def test_extra_fields_forbidden(self):
        """WgcDefaults rejects unknown fields at construction time."""
        with pytest.raises(Exception):
            WgcDefaults(nonexistent_field=42)
...
class TestTempConfig:
    ...
    def test_overrides_within_scope(self):
        original_timeout = wgc_config.defaults.batch_invocation_timeout
        with temp_config(batch_invocation_timeout=123.0):
            assert wgc_config.defaults.batch_invocation_timeout == 123.0
        assert wgc_config.defaults.batch_invocation_timeout == original_timeout

    def test_multiple_overrides(self):
        orig_timeout = wgc_config.defaults.task_llm_timeout
        orig_verbosity = wgc_config.defaults.verbosity
        with temp_config(task_llm_timeout=42.0, verbosity=0):
            assert wgc_config.defaults.task_llm_timeout == 42.0
            assert wgc_config.defaults.verbosity == 0
        assert wgc_config.defaults.task_llm_timeout == orig_timeout
        assert wgc_config.defaults.verbosity == orig_verbosity

    def test_restores_on_exception(self):
        """Values are restored even if the block raises."""
        original = wgc_config.defaults.num_retries
        with pytest.raises(RuntimeError):
            with temp_config(num_retries=99):
                assert wgc_config.defaults.num_retries == 99
                raise RuntimeError("boom")
        assert wgc_config.defaults.num_retries == original
    ...
    def test_nested_temp_config(self):
        ...
        original = wgc_config.defaults.max_rpm
        with temp_config(max_rpm=100):
            assert wgc_config.defaults.max_rpm == 100
            with temp_config(max_rpm=50):
                assert wgc_config.defaults.max_rpm == 50
            assert wgc_config.defaults.max_rpm == 100
        assert wgc_config.defaults.max_rpm == original
...
class TestResetToDefaults:
    ...
    def test_reset_after_mutation(self):
        wgc_config.defaults.task_llm_timeout = 999.0
        wgc_config.defaults.verbosity = 99
        wgc_config.reset_to_defaults()

        assert wgc_config.defaults.task_llm_timeout == 60.0
        assert wgc_config.defaults.verbosity == 1
