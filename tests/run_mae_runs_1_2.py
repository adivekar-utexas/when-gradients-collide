"""Run TextGrad val=mae experiments: runs 2 and 3.

Job groups:
1. Multi-task, val=mae, 4 modes (SSS/SSC/SCC/CCC) — runs 2 & 3
2. Single-task, val=mae, CCC mode, one task at a time — runs 2 & 3
...
def run_single(
    ...
    for k in ["task_model", "loss_model", "gradient_model", "optimizer_model"]:
        ...
        litellm.register_model({
            m["name"]: {
                # "litellm_provider": "openai",  # optional
...
def build_jobs(ds_config: Dict[str, Any]) -> List[JobSpec]:
    ...
    for run_n in RUNS:
        for mode_name in ["SSC"]:  # , "CCC", "SSS", , "SCC"
            job_name: str = (
                f"TextGrad-{ds_name}-{DATE_PREFIX}-{mode_name}-val=mae-run_{run_n}"
            )
            jobs.append((job_name, MODE_CONFIGS[mode_name], None, all_tasks))

        # NOTE: single-task CCC runs intentionally disabled for this script variant.
...
def run_dataset(ds_config: Dict[str, Any]) -> None:
    ...
    max_workers: int = min(len(jobs), 1)

    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        futures = {
            executor.submit(run_single, job_name, ds_class, mode_cfg, subset): (
                job_name,
                eval_tasks,
            )
    ...
    # job_name, mode_cfg, subset, eval_tasks = jobs[0]
    # out_dir = run_single(job_name, ds_class, mode_cfg, subset)
    # print("OUT:", out_dir)
    ...
    for job_name, _, _, _ in jobs:
        if job_name in errors:
            ...
            traceback.print_exception(type(errors[job_name]), errors[job_name], errors[job_name].__traceback__)
            continue
