"""
Setup script to download and prepare all datasets for WGC experiments.
...
Usage (after `pip install -e .`):
    python -m when_gradients_collide.expt.setup_datasets --datasets BRIGHTER
    python -m when_gradients_collide.expt.setup_datasets --datasets SummEval
"""

import argparse
import sys

from when_gradients_collide.expt.dataset import BRIGHTER, SummEval, WildGuard
...
def main():
    parser = argparse.ArgumentParser(
        description="Setup datasets for WGC experiments"
    )
    parser.add_argument(
        "--datasets",
        nargs="+",
